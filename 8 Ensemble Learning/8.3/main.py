...
- Date: 2021/06/16
- Brief: implement AdaBoost based on pruning-free decision tree with watermelon dataset 3.0α
"""
...
# use decision tree with depth 1 as the base learner
decisionTree = DecisionTree(xs, ys, xs, ys, attributes, isdiscs, labels)
decisionTree.setup(MaxDepth=1, partIndex='InformationGain', prepruning=False)

plt.figure()
base_nums = [5, 10, 20]
for it in range(len(base_nums)):
    adaboost = AdaBoost(decisionTree, base_nums[it])
    adaboost.fit(xs, ys)
    print("- Decision Tree (max depth -- 1) x %d\t:    %.2f"%(base_nums[it], adaboost.test(xs, ys)))
    ...
    preds = np.array([adaboost.classify(x) for x in datum]).reshape(x0.shape)
    plt.contour(x0, x1, preds, [0.5], linewidths=2, colors='m', label='Partition')

    plt.title("Decision Tree (max depth -- 1) x %d"%base_nums[it])
    plt.legend()
...
for it in range(len(base_nums)):
    adaboost = AdaBoost(decisionTree, base_nums[it])
    adaboost.fit(xs, ys)
    print("- Decision Tree (max depth -- 2) x %d\t:    %.2f"%(base_nums[it], adaboost.test(xs, ys)))
