...
- Date: 2020.10.27
- Brief: A Decision Tree class. Decision Tree is constructed with Stack
instead of recursive algorithm, which prevents stack overflow.
"""
...
class DecisionTree():
    ...
    def setup(self, MaxDepth, partIndex='InformationGain', prepruning=False):
        """ setup rules for building tree """
        self.MaxDepth = MaxDepth
        self.partIndex = partIndex
        self.prepruning = prepruning

    def fit(self, train_xs, train_ys):
        """ an interface to fit new dataset (Note: new dataset's attributes can't be changed) """
        self.train_xs = train_xs
        self.train_ys = train_ys
        self._build_tree()

    def buildTree(self, MaxDepth, partIndex='InformationGain', prepruning=False):
        """ build decision tree """
        self.MaxDepth = MaxDepth
        self.partIndex = partIndex
        self.prepruning = prepruning
    ...
    def _build_tree(self):
        ...
        self._stack = Stack()
        self._stack.push([self.handle, 0, self.train_xs, self.train_ys, self.attributes, 0])
        self._build_tree_with_stack()

        # root of Decision Tree
        self.root = self.handle.referChild(0)

    def _build_tree_with_stack(self):
        """ build decision tree with Stack """

        while not self._stack.empty():
            # partition isn't necessary, just insert leaf node
            if len(self._stack.top()) == 4:
                father, branch_val, leaf, cur_depth = self._stack.pop()
                father.insertChild(branch_val, leaf)
                continue
            # partition is necessary
            elif len(self._stack.top()) == 6:
                father, branch_val, xs, ys, partAttrs, cur_depth = self._stack.pop()

            # insert leaf node when all the xs are in the same class
            if len(set(ys)) == 1:
                father.insertChild(branch_val, LeafNode(ys[0]))
                continue

            # insert leaf node labeled mode of ys when partAttrs is empty or the node reaches MaxDepth
            if len(partAttrs) == 0 or cur_depth == self.MaxDepth:
                father.insertChild(branch_val, LeafNode(np.argmax(np.bincount(ys))))
                continue

            # insert leaf node labeled mode of ys when values of xs in partAttrs are the same
            temp = 0
            ...
            if isdisc:
                ...
                if self.prepruning:                             # pre-pruning
                    ...
                    # reset after computing accuracy
                    thisNode.childs = []
                ...
                for val in vals:
                    ...
                    if len(sub_ys) == 0:
                        self._stack.push([thisNode, val, LeafNode(np.argmax(np.bincount(ys))), cur_depth+1])
                    else:
                        branch_partAttrs = list(partAttrs)
                        branch_partAttrs.remove(opt_attr)
                        self._stack.push([thisNode, val, sub_xs, sub_ys, branch_partAttrs, cur_depth+1])
            # if the optimal attribute is continuous
                ...
                ys_less = ys[xs[:, opt_idx].astype(np.float64)<=thres]
                if len(ys_less) == 0:
                    self._stack.push([thisNode, 0, LeafNode(np.argmax(np.bincount(ys))), cur_depth+1])
                else:
                    self._stack.push([thisNode, 0, xs_less, ys_less, partAttrs, cur_depth+1])
                # for branch that value of the attribute is greater than threshold
                xs_greater = xs[xs[:, opt_idx].astype(np.float64)>thres]
                ys_greater = ys[xs[:, opt_idx].astype(np.float64)>thres]
                if len(ys_greater) == 0:
                    self._stack.push([thisNode, 1, LeafNode(np.argmax(np.bincount(ys))), cur_depth+1])
                else:
                    self._stack.push([thisNode, 1, xs_greater, ys_greater, partAttrs, cur_depth+1])
    ...
    def post_pruning_recursive(self, xs, ys, father, val, node):
        ...
        if type(node) == DiscreteNode:
            vals = node.attr_vals
            nheightss = [-child.height() for child in node.childs]
