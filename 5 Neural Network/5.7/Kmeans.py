...
class Kmeans():
    ...
    def cluster(self, xs):
        ...
        cnt = 1
        print("Clustering"+"."*cnt, end='', flush=True)

        np.random.shuffle(xs)
        stop = False
        while not stop:
            # init clusters
            clusters = [list() for i in range(self.k)]
            # put each x into nearest cluster
            for j in range(xs.shape[0]):
                nearest_idx = np.argmin(np.sum((self.means-xs[j])**2, axis=1))
                clusters[nearest_idx].append(xs[j])
            # calculate new means
            new_means = np.array([np.mean(np.array(clusters[i]), axis=0) for i in range(self.k)])
            stop = (new_means == self.means).all()
            self.means = new_means

            cnt = cnt%6 + 1
            print("\rClustering"+"."*cnt, end='', flush=True)

        print("\nEnd.\n")
        return clusters, self.means