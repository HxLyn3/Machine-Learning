...
- Date: 2020.10.20
- Brief: A Decision Tree class. You can choose optimal attribute for partition based on
Information Gain, Gini Index or Logit Regression when constructing the Decision Tree.
What's more, you can choose to use pre-pruning, post-pruning or not.
"""
...
class DecisionTree():
            ...
            for i in range(len(self.attr_values)):
                if attr_values[i]:
                    ...
                    for j in range(dims):
                        vec = [0]*dims
                        vec[j] = 1
                        ValMap[self.attr_values[i][j]] = vec
                    self.vals_map[i] = ValMap
    ...
    def opt_partition(self, xs, ys, partAttrs, index='InformationGain'):
        """ select the optimal attribute for partition """
        # index: partition index, 'InformationGain', 'GiniIndex' or 'LogitRegression'
        ...
        elif index == 'LogitRegression':            # based on accuracy of LogitRegression
            LogitRegAccs = [0]*len(partAttrs)
            for i in range(len(partAttrs)):
                # print(partAttrs[i])
                idx = list(self.attributes).index(partAttrs[i])
                # if the attribute is discrete
                if self.isdiscs[idx]:
                    LogitRegX = np.array([self.vals_map[idx][val] for val in xs[:, idx]]).astype(np.float64)
                    dims = len(LogitRegX[0])
                # if the attribute is continuous
                else:
                    LogitRegX = np.array([[np.float64(val)] for val in xs[:, idx]])
                    dims = 1
                LogitRegY = ys
                LogitRegressor = LogitReg(xsize=dims)
                LogitRegressor.load(LogitRegX, LogitRegY)
                LogitRegressor.learn(steps=30000, visible=False)
                LogitRegAccs[i] = LogitRegressor.test(LogitRegX, LogitRegY)
                if not self.isdiscs[idx]:
                    self.contiThres[partAttrs[i]] = -LogitRegressor.b/LogitRegressor.w[0]
            opt_attr = partAttrs[np.argmax(LogitRegAccs)]
