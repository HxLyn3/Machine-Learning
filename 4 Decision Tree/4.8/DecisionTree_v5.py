...
- Date: 2020.10.28
- Brief: A Decision Tree class. Decision Tree is constructed with Queue
instead of recursive algorithm, which aims to control the number of nodes.
"""
...
class DecisionTree():
    ...
    def buildTree(self, MaxNodes, partIndex='InformationGain', prepruning=False):
        """ build decision tree """
        ...
        self.node_cnt = 0
        self._queue = Queue()
        self._queue.push([self.handle, 0, self.train_xs, self.train_ys, self.attributes])
        self.build_tree_with_queue()

        # root of Decision Tree
        self.root = self.handle.referChild(0)

    def build_tree_with_queue(self):
        """ build decision tree with Queue """

        store_q = Queue()

        while not self._queue.empty():
            # partition isn't necessary, just insert leaf node
            if len(self._queue.front()) == 3:
                father, branch_val, leaf = self._queue.pop()
                father.insertChild(branch_val, leaf)
                self.node_cnt += 1
                continue
            # partition is necessary
            elif len(self._queue.front()) == 5:
                father, branch_val, xs, ys, partAttrs = self._queue.pop()
                self.node_cnt += 1

            # if the number of nodes reaches MaxNodes
            if self.node_cnt == self.MaxNodes:
            ...
            # insert leaf node labeled mode of ys when partAttrs is empty
            if len(partAttrs) == 0:
                father.insertChild(branch_val, LeafNode(np.argmax(np.bincount(ys))))
                continue
            ...
            if isdisc:
                ...
                father.insertChild(branch_val, thisNode)
                store_q.push([father, branch_val, thisNode, np.argmax(np.bincount(ys))])
                for val in vals:
                    sub_xs = xs[xs[:, opt_idx]==val]
                    sub_ys = ys[xs[:, opt_idx]==val]
                    # if the dataset of the branch is empty
                    if len(sub_ys) == 0:
                        self._queue.push([thisNode, val, LeafNode(np.argmax(np.bincount(ys)))])
                    else:
                        branch_partAttrs = list(partAttrs)
                        branch_partAttrs.remove(opt_attr)
                        self._queue.push([thisNode, val, sub_xs, sub_ys, branch_partAttrs])
            # if the optimal attribute is continuous
                ...
                ys_less = ys[xs[:, opt_idx].astype(np.float64)<=thres]
                if len(ys_less) == 0:
                    self._queue.push([thisNode, 0, LeafNode(np.argmax(np.bincount(ys)))])
                else:
                    self._queue.push([thisNode, 0, xs_less, ys_less, partAttrs])
                # for branch that value of the attribute is greater than threshold
                xs_greater = xs[xs[:, opt_idx].astype(np.float64)>thres]
                ys_greater = ys[xs[:, opt_idx].astype(np.float64)>thres]
                if len(ys_greater) == 0:
                    self._queue.push([thisNode, 1, LeafNode(np.argmax(np.bincount(ys)))])
                else:
                    self._queue.push([thisNode, 1, xs_greater, ys_greater, partAttrs])

        # fix the node at the last layer
        while not store_q.empty():
            father, branch_val, node, lbl = store_q.pop()
            if None in node.childs:
                father.insertChild(branch_val, LeafNode(lbl))
